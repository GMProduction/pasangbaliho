...
import { loadNegosiasiMateriById, setFinishNego, loadMediaOnUsed } from '../../Controller/NegosiasiControll';
...
export class PageNegoSelesai extends Component {
    ...
    initData = async () => {
        ...
        let data = await loadNegosiasiMateriById(id);
    ...
    update = async () => {
        ...
        let res = await setFinishNego(dataUpdate);
        if (res.status === 'ok') {
            this.setState({
                toNegoMateri: true
            })
    ...
    render() {
        ...
        if (this.state.toNegoMateri === true) {
            return <Redirect to='/admin/negosiasi/negomateri' />
        }
        return (
            <div>
                <LoadingBar
                ...
                <Grid item xs={12} sm={12} md={12} lg={6}>
                    <BasicPanel>
                        ...
                        <TextField type='number' name='hargaDeal' id="hargaDeal" label="Harga yang Di Sepakati" margin="dense" variant="outlined" fullWidth 
                                InputProps={{style: style.resize}} InputLabelProps={{style: style.resize}} value={this.state.hargaDeal} />
...
export default withStyles(breadcumbStyle)(PageNegoSelesai);
